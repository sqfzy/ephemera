...
fn render_asks(f: &mut Frame, book: &ephemera_data::BookData, area: Rect) {
    let header = Row::new(vec!["Price", "Amount", "Total"])
        .style(Style::default().fg(Color::Red).add_modifier(Modifier::BOLD))
        .bottom_margin(1);

    let max_amount = book.asks.iter()
        .map(|(_, amt)| amt.to_f64().unwrap_or(0.0))
        .fold(0.0_f64, f64::max);

    let rows: Vec<Row> = book.asks.iter().take(10).map(|(price, amount)| {
...
fn render_bids(f: &mut Frame, book: &ephemera_data::BookData, area: Rect) {
    let header = Row::new(vec!["Price", "Amount", "Total"])
        .style(Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))
        .bottom_margin(1);

    let max_amount = book.bids.iter()
        .map(|(_, amt)| amt.to_f64().unwrap_or(0.0))
        .fold(0.0_f64, f64::max);

    let rows: Vec<Row> = book.bids.iter().take(10).map(|(price, amount)| {
        let price_f = price.to_f64().unwrap_or(0.0);
        let amount_f = amount.to_f64().unwrap_or(0.0);
        let total = price_f * amount_f;
...
fn render_spread_info(f: &mut Frame, book: &ephemera_data::BookData, area: Rect) {
